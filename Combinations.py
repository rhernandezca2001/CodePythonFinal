...
def read_input_RS ():
    ...
    upper_bound = np.ravel(np.transpose(np.loadtxt('UpperArray.csv', delimiter=',', skiprows=1)))
...
def independent_probability ():
    probability_assignment = np.loadtxt('ProbabilityAssignment.csv', delimiter=',', skiprows=1)
